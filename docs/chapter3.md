...
The intelligence that drives Physical AI comes from machine learning models trained to perceive, plan, and act in the physical world. This chapter explores the major paradigms for robot learning, from reinforcement learning to foundation models.

## Reinforcement Learning for Robotics

Reinforcement Learning (RL) enables robots to discover optimal behaviors through trial and error, learning from the consequences of their actions rather than explicit instruction.
...
- **Action**: Commands sent to actuators
...
Deep neural networks enable RL to scale to high-dimensional observations and complex behaviors:

**Deep Q-Networks (DQN)** learn to estimate the value of taking each action in each state. This approach works for discrete action spaces.
...
- **PPO** (Proximal Policy Optimization): Constrains policy updates for stable learning
- **SAC** (Soft Actor-Critic): Maximizes entropy alongside reward for exploration
...
**Sim-to-Real Gap**: Training in simulation is much faster, but behaviors don't always transfer to the real world due to modeling inaccuracies.
...
3. Train neural network to map states to actions
...
Rather than imitating actions, IRL infers the reward function the expert is optimizing. This recovered reward can then train RL agents or evaluate new behaviors.
...
**Generative Adversarial Imitation Learning (GAIL)** frames IRL as a GAN problem where the generator tries to match expert behavior distribution.

IRL is computationally expensive but can enable generalization beyond demonstrated scenarios.

### Teleoperation Systems

Teleoperation allows humans to control robots remotely, simultaneously enabling task execution and data collection:
...
**RT-1** (Robotics Transformer 1) from Google trains a vision-language-action model on diverse robot tasks. The model takes image observations and language instructions, outputting robot actions. Training on 130k demonstrations across 700+ tasks enables generalization to new scenarios.

**RT-2** extends this by fine-tuning web-scale vision-language models (PaLI-X) for robotics. This grounds internet knowledge in robot actions, dramatically improving reasoning and generalization. RT-2 can follow instructions requiring world knowledge that was never demonstrated.
...
**Octo** is an open-source generalist robot policy trained on 800k trajectories from multiple robot platforms. It provides a foundation that can be fine-tuned for specific robots and tasks.

### Benefits of Foundation Models

- **Sample Efficiency**: Pre-training on large datasets provides a strong starting point
- **Transfer Learning**: Knowledge generalizes across tasks and embodiments
...
Training in simulation is much faster than the real world, but simulation cannot perfectly capture physical reality. Bridging this "reality gap" is crucial for practical Physical AI.
...
**Neural Radiance Fields (NeRFs)** and **Gaussian Splatting** create photorealistic 3D scenes from multi-view images. These can serve as training environments that precisely match reality.

**Digital Twins** that continuously update from real-world sensors provide increasingly accurate simulation.
...
- **MAML** (Model-Agnostic Meta-Learning): Fine-tunes in few steps
...
**Next Chapter**: [The Rise of Humanoid Robots →](/chapter4)